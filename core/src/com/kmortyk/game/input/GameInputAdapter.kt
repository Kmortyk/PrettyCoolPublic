...
class GameInputAdapter(
    ...
    private val gameState: GameState) : InputAdapter() {
    ...
    override fun touchDown(screenX: Int, screenY: Int, pointer: Int, button: Int): Boolean {
        ...
        if (!ui.touch(uiPos.x, uiPos.y)) {
            // update game state with touch TODO float ?
            gameState.touch(worldPos.x.toInt(), worldPos.y.toInt())
        }
        // consume value
        return true
    }
    ...
    private fun uiPos(screenX: Int, screenY: Int) : Vector2 {
        ...
        if(PrettyCoolGame.Debug && PrettyCoolGame.DrawTouchedPosGame) {
